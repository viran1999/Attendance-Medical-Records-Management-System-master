        ...
        <div class="row">
            <div class="col-md-2" style="margin:0px 0px 0px 0px ;  ">    
                ...
                <a class="btn btn-outline-light" href="/ARview" style="border-bottom:6px solid #000" >Send To UMO</a>
                    <a class="btn btn-outline-light" href="#" style="border-bottom:6px solid #000">View medicals </a>
                    <a class="btn btn-outline-light" href="/ARexam">View exam medicals sent to faculty board</a>
                    <a class="btn btn-outline-light" href="/search">view Medical Reports</a>
                </div>
            ...
            <div class="col-md-10" style="margin:0px 0px 0px 0px ;  ">
                ...
                <fieldset>
                    <legend><b>View MEDICAL RECORDS:</b></legend>
                    <label for="Sid">Enter the Student ID:</label>
                    <input type="text" id="Sid" name="Sid">
                    <button type="button" class="btn btn-primary" onclick="search();">Search</button><br><br>
                ...
                <!--    </div>-->
                    ...
                    <label class="form-check-label" for="send to UMO">UMO accepted</label><br>
                    <input type="checkbox" id="rejected" class="form-check-input">
                    <label class="form-check-label" for="reject">UMO rejected</label><br><br>
                </div>
...
<script type="text/javascript">
                        ...
                        function search(){          
                            ...
                            var docRef = db1.collection("addmedical").doc(Sidv);

                            docRef.get().then(function(doc) {
                            if (doc.exists) {
                            if(doc.data().approved===false && doc.data().rejected===false && doc.data().status==="UMO"){
                            if (doc.data().lecture===true ) {
                           // console.log("Document data:", doc.data());
                            document.getElementById("name").value=doc.data().name;
                            document.getElementById("DoS").value=doc.data().DoS;
                            document.getElementById("reason").value="lectures";
                            }else if (doc.data().exam===true) {
                           // console.log("Document data:", doc.data());
                            document.getElementById("name").value=doc.data().name;
                            document.getElementById("DoS").value=doc.data().DoS;
                            document.getElementById("reason").value="exams";
                            }
                            }else{
                        ...
                        function send(){
                            ...
                            const rejected = document.getElementById("rejected").checked;
                            const reason  =  document.getElementById("reason").value;
                            if(accepted===true && reason==="lectures"){
                                db1.collection("addmedical").doc(Sidv).update({
                                approved: true
                                })
                                .then(function() {
                                ...
                                console.log("lecs");
                            }else if(accepted===true && reason==="exams"){
                                db1.collection("addmedical").doc(Sidv).update({
                                status: "faculty board"
                                })
                                .then(function() {
                                console.log("Document successfully written!");
                                })
